...
@st.cache_data
def load_data():
    return pd.read_csv("data/processed/mcsa_dataset.csv")
...
with tab1:
    ...
    if st.button("🔮 Предсказать RUL", type="primary", use_container_width=True):
        ...
        # Сохраняем результат в session_state (чтобы кнопка сохранения видела его)
        st.session_state.last_prediction = {
            ...
            'status': "Критично" if prediction < 100 else "Риск" if prediction < 200 else "Норма"
        }
        ...
        with col_c:
            st.info("Параметры успешно обработаны")
    ...
    if 'last_prediction' in st.session_state:
        if st.button("💾 Сохранить прогноз в историю", type="secondary", use_container_width=True):
            ...
            # Очищаем последнее предсказание после сохранения
...
st.success("✅ День 7 завершён — дашборд полностью готов к использованию!")
